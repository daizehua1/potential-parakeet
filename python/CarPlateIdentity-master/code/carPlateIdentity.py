...
sys.path.append(r"E:\python_object\chepaishibie\venv\Lib\site-packages")
import os
...
def pre_process(orig_img):
    ...
    # 灰度处理
    cv2.imshow('gray_img111', gray_img)
    blur_img = cv2.blur(gray_img, (3, 3))
    # 柔化减少噪声
    cv2.imshow('blur111', blur_img)
    # 计算x方向上的梯度可以检测图像中垂直于x轴的边缘。梯度是一个向量，它的方向指向灰度值变化最快的方向，
    # 大小表示灰度值变化的速度。在边缘处，灰度值会发生剧烈变化，因此梯度的大小会很大。通过计算x方向上的梯度，
    # 我们可以找到图像中垂直于x轴的边缘。
    ...
    cv2.imshow('blue_img111', blue_img)
...
# 给候选车牌区域做漫水填充算法，一方面补全上一步求轮廓可能存在轮廓歪曲的问题， 另一方面也可以将非车牌区排除掉 上面的代码是一个名为 `verify_color` 的函数，它使用 OpenCV
# 的漫水填充算法来填充一个掩码图像。该函数接受两个参数：`rotate_rect` 和 `src_image`。`rotate_rect` 是一个旋转矩形，表示要填充的区域。`src_image` 是原始图像。
# 首先，函数创建了一个与原始图像大小相同的掩码图像，并初始化了一些参数，例如 `connectivity`、`loDiff`、`upDiff` 和 `flags`。然后，函数使用 `cv2.boxPoints`
# 函数获取旋转矩形的四个顶点，并计算出随机种子点的行和列范围。
# 接下来，函数检查种子点在水平或垂直方向上可移动的范围是否很小。如果是，则使用旋转矩阵对角线来设置随机种子点。否则，使用 `np.random.randint` 和 `np.linspace` 函数在指定范围内生成随机种子点。
# 最后，函数将原始图像转换为 HSV 颜色空间，并在一个循环中对每个种子点进行漫水填充。如果种子点的颜色在指定范围内，则使用 `cv2.floodFill` 函数填充掩码图像。
...
def locate_carPlate(orig_img, pred_image):
    ...
    for i, contour in enumerate(contours):
        ...
        if verify_scale(rotate_rect):
            ...
            if ret == False:
                continue
            # cv2.imshow('no_Transform_car_plate', rotate_rect2)
            # 车牌位置矫正
            ...
            cv2.imshow('Transform_car_plate', car_plate)
            # ========================调试看效果========================#
...
def horizontal_cut_chars(plate):
    ...
    col_limit = 0  # round(0.5*sum/img_w)cf
    # 每个字符宽度也进行限制
    ...
    for i in range(img_w):
        ...
        if colValue > col_limit:
            if is_char_flag == False:
                area_right = round((i + char_right) / 2)  # 区域的右边界位置（当前列和上一个字符的右边界的中间位置）
...
def get_chars(car_plate):
    ...
    for i, addr in enumerate(char_addr_list):
        ...
        i = str(i)
        cv2.imwrite('./carIdentityData/opencv_output/1/' + i + '.jpg', char_img)
        cv2.imshow('char', char_img)

    return char_imgs


def extract_char(car_plate, color):
    gray_plate = cv2.cvtColor(car_plate, cv2.COLOR_BGR2GRAY)
    if color == 1:
        ret_white, binary_plate_white = cv2.threshold(gray_plate, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        char_img_list = get_chars(binary_plate_white)

    # 识别黑色字符
    if color == 0:
        ret_black, binary_plate_black = cv2.threshold(gray_plate, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
        char_img_list = get_chars(binary_plate_black)
...
#     h, s, v = hsv_img[:, :, 0], hsv_img[:, :, 1], hsv_img[:, :, 2]
#     # 黄色色调区间[26，34],蓝色色调区间:[100,124]
#     blue_img = ((h > 100) & (h < 124)) & (s > 70) & (v > 70)
#     yellow_img = (h > 11) & (h < 34) & (s > 70) & (v > 70)
#     blue_img = blue_img.astype('float32')
#     yellow_img = yellow_img.astype('float32')
#     mix_img_blue = np.multiply(sobel_img, blue_img)
#     mix_img_yellow = np.multiply(sobel_img, yellow_img)
#     cnt_array_blue = np.where(mix_img_blue, 1, 0)
#     cnt_array_yellow = np.where(mix_img_yellow, 1, 0)
#     if (np.sum(cnt_array_blue) > (np.sum(cnt_array_yellow))):
#         return 1
#     elif (np.sum(cnt_array_blue) < (np.sum(cnt_array_yellow))):
#         return 0
#     else:
#         return 0
...
def check_color(car_plate):
    ...
    blue_img = ((h > 100) & (h < 124)) & (s > 70) & (v > 70)
    yellow_img = ((h > 11) & (h < 34)) & (s > 70) & (v > 70)
    green_img = ((h > 35) & (h < 77)) & (s > 70) & (v > 70)
    blue_img = blue_img.astype('float32')
    yellow_img = yellow_img.astype('float32')
    green_img=green_img.astype('float32')
    if (np.sum(blue_img) > (np.sum(yellow_img))):
        return 1
    elif (np.sum(blue_img) < (np.sum(yellow_img))):
        return 0
    elif (np.sum(green_img)):
        return 0
...
if __name__ == '__main__':
    ...
    img = cv2.imread('./images/pictures/25.jpg')
    cv2.imshow('org', img)
    # 预处
    ...
    # 判断车牌的颜色
    color = check_color(car_plate)

    # 字符提取
    char_img_list = extract_char(car_plate, color)

...
class char_cnn_net:
    ...
    def cnn_construct(self):
        ...
        cw1 = tf.Variable(tf.random_normal(shape=[3, 3, 1, 32], stddev=0.01), dtype=tf.float32)#卷积核
        cb1 = tf.Variable(tf.random_normal(shape=[32]), dtype=tf.float32)#偏置
        conv1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(x_input,filter=cw1,strides=[1,1,1,1],padding='SAME'),cb1)) #卷积输出
    ...
    def train(self,data_dir,save_model_path):
        ...
        # 计算损失函数，使用softmax交叉熵函数计算预测结果和真实标签之间的损失
        opt = tf.train.AdamOptimizer(learning_rate=0.001)
        # Adam
        # 优化器是一种常用的梯度下降优化算法，它根据损失函数的梯度信息来更新模型的参数，使得损失函数的值逐渐减小。
        # 它具有自适应学习率的特性，可以在训练过程中自动调整学习率，以便更好地适应不同参数的更新情况。
        train_step = opt.minimize(cost)
        # opt.minimize(cost)
        # 的作用是根据定义的优化器和损失函数，自动计算梯度并更新模型的参数，以使损失函数的值尽可能地减小。
        # 具体来说，它会根据损失函数的梯度信息，按照优化器定义的更新规则，更新模型中所有可训练的参数，使得损失函数在训练过程中逐渐下降。
...
if __name__ == '__main__':
    ...
    model_path = os.path.join(cur_dir,'./carIdentityData/model/char_recongnize/model.ckpt-630')
